...
""";
    ...
    public static Config load(Path dir) throws IOException {
        var wofloDir = dir.resolve("woflo"); Files.createDirectories(wofloDir);
        var f = wofloDir.resolve("woflo.yml"); var c = new Config();
        if (!Files.exists(f)) { Files.writeString(f, DEFAULT); return c; }
        c.parse(Files.readString(f)); return c;
    }

    private void parse(String content) {
        String section = "", listKey = ""; boolean jvmCleared = false;
        for (var raw : content.split("\n")) {
            var line = raw.trim();
            if (line.isEmpty() || line.startsWith("#")) continue;
            int indent = 0; for (var ch : raw.toCharArray()) { if (ch == ' ') indent++; else break; }
            if (line.startsWith("- ")) { if (listKey.equals("jvm-args")) { if (!jvmCleared) { jvmArgs.clear(); jvmCleared = true; } jvmArgs.add(line.substring(2).trim()); } continue; }
            int col = line.indexOf(':'); if (col == -1) continue;
            String key = line.substring(0, col).trim(), val = line.substring(col + 1).trim();
            int cmt = val.indexOf('#'); if (cmt > 0) val = val.substring(0, cmt).trim();
            if (val.startsWith("\"") && val.endsWith("\"")) val = val.substring(1, val.length() - 1);
            if (val.isEmpty()) { if (indent == 0) section = key; else listKey = key; continue; }
            switch (section) {
                case "memory" -> { if (key.equals("min")) memoryMin = val; else if (key.equals("max")) memoryMax = val; }
                case "updates" -> { switch (key) { case "minecraft" -> updateMinecraft = bool(val); case "mods" -> updateMods = bool(val); case "plugins" -> updatePlugins = bool(val); case "datapacks" -> updateDatapacks = bool(val); case "min-compatibility" -> minCompatibility = num(val, 60); case "allow-snapshots" -> allowSnapshots = bool(val); case "allow-beta" -> allowBeta = bool(val); } }
                case "" -> {
                    switch (key) {
    ...
    private boolean bool(String s) { return s.equalsIgnoreCase("true") || s.equalsIgnoreCase("yes"); }
    private int num(String s, int d) { try { return Integer.parseInt(s); } catch (Exception e) { return d; } }
}
