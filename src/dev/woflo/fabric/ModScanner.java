...
public class ModScanner {
    ...
    public static void init(Path woflo, Console c) {
        wofloDir = woflo;
        console = c;
        ...
        migrate(server.resolve(worldName(server)).resolve("datapacks").resolve("datapacks.txt"),
                woflo.resolve("datapacks.txt"));
    }
    ...
    private static void migrate(Path src, Path dest) {
        try {
            if (Files.exists(src) && !Files.exists(dest)) {
                Files.move(src, dest);
                if (console != null)
                    console.info("Moved " + src.getFileName() + " to woflo/");
            }
        } catch (IOException ignored) {
        }
    }
    ...
    private static List<Mod> scanDir(Path dir, String ext, String skipFile, String label, Function<Path, String> reader,
        ...
        try (var ex = Executors.newVirtualThreadPerTaskExecutor()) {
            var futures = list(dir, ext).stream().map(p -> ex.submit(() -> {
                ...
                return (id != null && !id.isEmpty() && !ignored.contains(id.toLowerCase()))
                        ? new Mod(id, p, p.getFileName().toString())
                        : null;
            })).toList();
            all = futures.stream().map(f -> {
                try {
                    return f.get();
                } catch (Exception e) {
                    return null;
    ...
    private static String readPluginId(Path jar) {
        try (var z = new ZipFile(jar.toFile())) {
            ...
            if (id == null)
                id = yaml(z, "bungee.yml", "name");
            if (id == null)
                id = json(z, "velocity-plugin.json", "id");
            return id;
        } catch (Exception e) {
            return null;
        }
    ...
    private static String readDatapackId(Path zip) {
        try (var z = new ZipFile(zip.toFile())) {
            ...
            if (id == null) {
                ...
                if (pack != null)
                    id = Http.str(Http.obj(pack, "pack"), "id");
            }
            if (id != null)
                return id;
    ...
    private static Map<String, Object> jsonObj(ZipFile z, String file) {
        ...
        if (entry == null)
            return null;
        try (var is = z.getInputStream(entry)) {
            return Http.parseObject(new String(is.readAllBytes()));
        } catch (Exception e) {
            return null;
        }
    }
    ...
    private static String genSkipFile(List<Mod> items, Set<String> skipped, String label) {
        ...
        items.stream().sorted(Comparator.comparing(Mod::fileName)).forEach(m -> sb
                .append(skipped.contains(m.fileName().toLowerCase()) ? "# " : "").append(m.fileName()).append("\n"));
        return sb.toString();
    }

    private static void writeIfChanged(Path f, String content) {
        try {
            if (!content.equals(Files.exists(f) ? Files.readString(f) : ""))
    ...
    private static List<Path> list(Path dir, String ext) throws IOException {
        try (var s = Files.list(dir)) {
            return s.filter(p -> p.toString().endsWith(ext)).toList();
        }
    }
}
